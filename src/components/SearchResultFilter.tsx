    ...
    <>
              ...
              className={`relative z-10 px-1.5 py-0.5 sm:px-2 sm:py-1 md:px-4 md:py-2 text-xs sm:text-sm font-medium rounded-full transition-all duration-200 whitespace-nowrap ${
                activeCategory === category.key
                  ? isDefaultValue(category.key)
                    ...
                    : 'text-theme-primary cursor-default'
                  : isDefaultValue(category.key)
                  ? 'text-gray-700 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100 cursor-pointer'
                  : 'text-theme-primary hover:text-theme-primary-hover cursor-pointer'
              }`}
            >
      ...
      {activeCategory &&
        createPortal(
          <div
            ref={dropdownRef}
            className='fixed z-[9999] bg-theme-surface/95 rounded-xl border border-theme-border backdrop-blur-sm max-h-[50vh] flex flex-col'
            style={{
                    ...
                    <button
                      ...
                      className={`px-2 py-1.5 sm:px-3 sm:py-2 text-xs sm:text-sm rounded-lg transition-all duration-200 text-left ${
                        isOptionSelected(activeCategory, option.value)
                          ? 'bg-theme-primary-soft text-theme-primary border border-theme-primary/25'
