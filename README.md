...
CraterTV 使用独立的语义化版本号，当前版本为 `1.4.3`。上游版本只用于标记同步基线，不再作为 CraterTV 的发布版本号。
...
| 安全搜索     | 用户组可启用影视库安全搜索。原始关键词只发送给每个允许的采集源一次；豆瓣或 TMDB 的合规标题只要是返回标题的前缀即可通过，无法确认合规的结果不显示。                                                  |
...
- **简化安全标题匹配**：影视库合规标题只要是播放站结果的前缀即可通过，允许续集和各种版本后缀。
- **修正安全搜索误过滤**：豆瓣验证优先使用可部署的 CDN，并允许“加更版”与对应规范标题匹配。
