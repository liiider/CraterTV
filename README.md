...
CraterTV 使用独立的语义化版本号，当前版本为 `1.4.2`。上游版本只用于标记同步基线，不再作为 CraterTV 的发布版本号。
...
| 安全搜索     | 用户组可启用影视库安全搜索。原始关键词只发送给每个允许的采集源一次；返回标题先由豆瓣精确验证（允许“加更版”后缀），未命中时再由 TMDB 排除成人内容。无法确认合规的结果不显示。                        |
