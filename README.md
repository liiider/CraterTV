...
CraterTV 使用独立的语义化版本号，当前版本为 `1.4.6`。上游版本只用于标记同步基线，不再作为 CraterTV 的发布版本号。
...
| 安全搜索     | 用户组可启用影视库安全搜索。原始关键词只发送给每个允许的采集源一次；来源带正整数豆瓣 ID 时直接放行，否则按返回标题依次由豆瓣和 TMDB 规范化精确验证。无法确认合规的结果不显示。                      |
| 配置保护     | 数据库或 Redis 读取失败时返回错误，不把读取异常当成空配置，不写回默认配置。                                                                                                                         |
| 搜索与源检测 | 搜索优先请求电影天堂；采集源分批并发，单个源保留 8 秒超时且失败不影响其他源；成功、空结果、403 和超时响应均不缓存，下次搜索会重新请求。                                                             |
...
- **安全搜索优先使用豆瓣 ID**：来源返回正整数豆瓣 ID 时直接放行，ID 缺失或无效时继续执行原有标题精确验证，避免来源季名与影视库规范名称不同导致误过滤。
- **搜索结果不再缓存**：成功、空结果、403 和超时响应都不会复用，下次搜索重新请求对应采集源；单个源继续保留 8 秒超时。
- **电影天堂来源优先**：搜索时优先请求电影天堂；播放候选中存在电影天堂资源时优先使用该来源。
- **修正新增用户即时登录**：配置缓存缺少刚新增用户时回源刷新，避免跨实例误判用户不存在并退出登录。
- **首页播放优先使用豆瓣 ID**：首页把豆瓣 ID 传到播放页；来源返回相同有效 ID 时直接视为同一影片，ID 缺失或不一致时保留原有标题、年份和类型规则。
...
| `TMDB_API_READ_TOKEN`                 | 安全搜索建议   | TMDB API 读访问令牌，豆瓣未精确命中返回标题时用于过滤成人内容         |
...
用户原始关键词分别发送给该组允许的播放源。播放源结果带正整数豆瓣 ID 时直接放行；
ID 缺失或无效时，再按返回标题查询豆瓣，未精确匹配则由 TMDB 排除成人内容。两个
影视库都不能确认合规的结果不显示。
...
- 播放源返回正整数豆瓣 ID 时优先放行，不再要求来源季名与影视库规范标题一致。
